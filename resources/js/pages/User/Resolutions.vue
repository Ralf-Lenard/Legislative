<template>
        ...
        >
          <div class="flex-1 text-white relative z-10">
            ...
            <p class="text-lg mt-3 text-gray-100">
              Formalizing decisions for community progress.
            </p>
          </div>
          ...
          >
            <div
              ...
              :class="{
                ...
                // 'lg:col-span-2': resolution.id % 5 === 1 // Making some cards wider to match the visual variety in the image
              }"
            >
  ...
  const props = defineProps({
    resolutions: Object, // Renamed from ordinances
    filters: Object,
    years: Array,
  ...
  const applyFilters = () => {
    ...
    // Ensure the route is /resolutions, not /ordinances
  ...
  //      MODAL LOGIC
  // ==========================
  const selectedResolution = ref(null); // Renamed from selectedOrdinance
  
  const openModal = (resolution) => { // Renamed parameter
  ...
  const openRequestModal = (resolution) => { // Renamed parameter
    if (!props.user) {
    ...
    selectedResolution.value = resolution; // Ensure the correct resolution is selected for the request
  ...
  const submitRequestForm = () => {
    ...
    console.log('Request submitted for resolution:', selectedResolution.value.resolutions_number, 'Purpose:', requestForm.purpose);
