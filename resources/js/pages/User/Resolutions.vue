<template>
  ...
  <div class="bg-white min-h-screen">
    ...
    <section class="py-14 px-4 bg-gray-50">
      <div class="max-w-7xl mx-auto">
        ...
        <div v-else class="text-center mt-16 bg-white p-10 rounded-xl shadow border border-gray-200">
          <h3 class="text-2xl font-bold text-gray-700">No Resolutions Found</h3>
        ...
        <div v-if="resolutions.data.length" class="mt-16 flex justify-center">
    ...
    <transition name="modal">
      <div
        v-if="selectedResolution && !showRequestModal"
      ...
      >
        ...
        >
          <div class="overflow-y-auto px-6 py-6 flex-1 custom-scrollbar">
            ...
            <h2 class="text-2xl font-bold text-green-900">Resolution No. {{ selectedResolution.resolutions_number }}</h2>
            <h3 class="text-lg font-semibold mt-2">{{ selectedResolution.title_resolutions }}</h3>
            <div class="flex flex-col gap-1 mt-2">
              <p class="text-sm text-gray-600">
                Date Approved: <strong>{{ formatDate(selectedResolution.date_approved_resolutions) }}</strong>
              </p>
            ...
            <div class="mt-6 rounded-lg overflow-hidden border border-gray-200 shadow" v-if="selectedResolution.image_resolutions">
              <img :src="`/storage/${selectedResolution.image_resolutions}`" class="w-full" />
            </div>

            <div class="mt-6">
              <h4 class="font-bold mb-1">Description:</h4>
              <p class="text-gray-700 leading-relaxed">{{ selectedResolution.description_resolutions }}</p>
            </div>
          </div>
...
const applyFilters = () => {
    router.get('/citizens-charter/resolutions', form, { preserveState: true, replace: true });
};
...
const openModal = (resolution) => { selectedResolution.value = resolution; };
const closeModal = () => { selectedResolution.value = null; };
...
const submitRequestForm = async () => {
    ...
    try {
        ...
        if (recaptchaEnabled.value) {
            ...
            token = await window.grecaptcha.execute(siteKey.value, { action: 'resolution_request' });
            if (!token) throw new Error('Could not generate reCAPTCHA token.');
        }

        router.post(`/resolutions/${currentResolutionId}/request-access`, {
            purpose: requestForm.purpose,
            valid_id_type: requestForm.valid_id_type,
            valid_id: requestForm.valid_id,
...
const handleDownloadClick = (resolution) => {
    ...
    console.log('Download resolution:', resolution.id);
};
</script>
