...
import RequestDetailsModal from '@/components/ModalRequest/RequestDetailsModalOrdinances.vue'; 
...
const paginationMeta = computed(() => page.props.requests?.meta);
...
const clearFilters = () => {
    search.value = '';
    statusFilter.value = '';
    router.get('/ordinance-request', {}, { replace: true });
};
...
const selectedRequest = ref<OrdinanceDownloadRequest | null>(null);
...
const handleOpenDetails = (request: OrdinanceDownloadRequest) => {
...
const submitApproval = () => {
    ...
    router.post(`/ordinance-request/${approvingRequest.value.id}/approve`, {}, {
        onFinish: () => {
...
const submitRejection = () => {
    ...
    router.post(`/ordinance-request/${rejectingRequest.value.id}/reject`, 
...
<template>
        ...
        <main class="relative flex-1 overflow-auto">
            ...
            <div class="sticky top-0 z-40 border-b border-slate-200 bg-white shadow-md">
                <div class="flex items-center justify-between px-8 py-6">
                    <h1 class="text-3xl font-extrabold text-slate-900">Ordinance Requests</h1>
                </div>
                        ...
                        <div class="flex flex-1 flex-col gap-4 md:flex-row md:items-center md:gap-3">
                            ...
                            <div class="relative max-w-sm flex-1">
                                ...
                                <input
                                    ...
                                    type="text"
                                    placeholder="Search by user, ordinance..."
                                    class="w-full rounded-lg border border-slate-300 bg-white py-2.5 pr-4 pl-10 text-slate-900 shadow-sm transition-all focus:border-transparent focus:ring-2 focus:ring-emerald-500 focus:outline-none"
                                />
                            </div>
                            ...
                            <div class="relative w-full md:w-48">
                                <select
                                    ...
                                    class="w-full rounded-lg border border-slate-300 bg-white px-4 pr-10 py-2.5 text-slate-900 outline-none focus:ring-2 focus:ring-emerald-500 appearance-none">
                            ...
                            <button
                                ...
                                @click="clearFilters"
                                class="flex items-center gap-2 rounded-lg border border-slate-300 bg-white px-4 py-2.5 font-medium text-slate-700 hover:bg-slate-50 transition-colors"
                            >
                                <X class="h-4 w-4" />
                                Clear
                            </button>
                ...
                <div class="overflow-hidden rounded-lg border border-slate-100 bg-white shadow-lg">
                    ...
                    <table v-else class="w-full divide-y divide-slate-200">
                        ...
                        <tbody class="divide-y divide-slate-200">
                            <tr v-for="(request, index) in requestsList" :key="request.id" class="hover:bg-emerald-50/50 bg-white transition-colors">
                                ...
                                <td class="flex justify-center gap-2 px-6 py-4">
                                    <button @click="handleOpenDetails(request)" class="flex h-8 w-8 items-center justify-center rounded-full bg-blue-50 text-blue-600 hover:bg-blue-100 transition-colors">
                                        <Eye class="h-4 w-4" />
                                    </button>
                                    <template v-if="request.status === 'pending'">
                                        <button @click="openApproveModal(request)" class="rounded-full bg-emerald-50 px-3 py-1 text-xs font-semibold text-emerald-600 hover:bg-emerald-100 transition-colors">Approve</button>
                                        <button @click="openRejectModal(request)" class="rounded-full bg-red-50 px-3 py-1 text-xs font-semibold text-red-600 hover:bg-red-100 transition-colors">Reject</button>
                                    </template>
                                </td>
                            </tr>
                        </tbody>
    ...
    <div v-if="isApproveModalOpen" class="fixed inset-0 z-[9999] flex items-center justify-center p-4">
        ...
        <div class="relative w-full max-w-md rounded-lg bg-white p-6 shadow-xl" @click.stop>
            ...
            <div class="flex justify-end gap-3">
                <button @click="isApproveModalOpen = false" class="rounded-lg bg-slate-100 px-4 py-2 text-slate-700 hover:bg-slate-200 transition-colors">Cancel</button>
                <button @click="submitApproval" class="rounded-lg bg-emerald-600 px-4 py-2 font-semibold text-white hover:bg-emerald-700 transition-colors">Yes, Approve</button>
            </div>
        </div>
    </div>
    ...
    <div v-if="isRejectModalOpen" class="fixed inset-0 z-[9999] flex items-center justify-center p-4">
        ...
        <div class="relative w-full max-w-md rounded-lg bg-white p-6 shadow-xl" @click.stop>
            ...
            <textarea v-model="rejectionReason" rows="4" class="w-full rounded-lg border border-slate-300 bg-white p-2 text-slate-900 focus:ring-2 focus:ring-red-500 focus:outline-none" placeholder="Enter reason..."></textarea>
            <div class="mt-4 flex justify-end gap-3">
                <button @click="isRejectModalOpen = false" class="rounded-lg bg-slate-100 px-4 py-2 text-slate-700 hover:bg-slate-200 transition-colors">Cancel</button>
                <button @click="submitRejection" class="rounded-lg bg-red-600 px-4 py-2 font-semibold text-white hover:bg-red-700 transition-colors">Reject</button>
            </div>
        </div>
    </div>
