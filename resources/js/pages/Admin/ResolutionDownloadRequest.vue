<script setup lang="ts">
  ...
  import { Eye, Search, X, FileText, MessageSquare, CheckCircle } from 'lucide-vue-next';
    ...
    <Head title="Resolution Requests" />
                      ...
                      <div class="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
                          <div class="flex flex-1 flex-col gap-4 md:flex-row md:items-center md:gap-3">
                              ...
                              <div class="relative">
                                  ...
                                  <svg class="pointer-events-none absolute top-1/2 right-3 h-5 w-5 -translate-y-1/2 text-slate-400" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                          ...
                          <tbody class="divide-y divide-slate-200">
                              <tr v-for="(request, index) in requests" :key="request.id" class="transition-colors hover:bg-emerald-50/50">
                                  ...
                                  <td class="flex justify-center gap-2 px-6 py-4 text-center">
                                      <button @click="openDetailsModal(request)" class="flex h-8 w-8 items-center justify-center rounded-full bg-blue-50 text-blue-600 shadow-sm hover:bg-blue-100">
                                          <Eye class="h-4 w-4" />
                                      </button>
                                      <button v-if="request.status === 'pending'" @click="openApproveModal(request)" class="flex h-8 items-center justify-center rounded-full bg-emerald-50 px-3 text-xs font-semibold text-emerald-600 shadow-sm hover:bg-emerald-100">
                                          Approve
                                      </button>
                                      <button v-if="request.status === 'pending'" @click="openRejectModal(request)" class="flex h-8 items-center justify-center rounded-full bg-red-50 px-3 text-xs font-semibold text-red-600 shadow-sm hover:bg-red-100">
                                          Reject
                                      </button>
                                  </td>
                              </tr>
                      ...
                      <div v-if="paginationLinks && paginationLinks.length > 3" class="flex items-center justify-between border-t border-slate-200 bg-slate-50 px-4 py-4 sm:px-6">
                          ...
                          <nav class="isolate inline-flex -space-x-px rounded-md shadow-sm">
                              <component
                                  ...
                                  :disabled="!link.url"
                                  :class="['relative inline-flex items-center px-4 py-2 text-sm font-semibold ring-1 ring-slate-300 transition-all ring-inset focus:z-20',
                                      key === 0 ? 'rounded-l-md' : '',
                                      key === paginationLinks.length - 1 ? 'rounded-r-md' : '',
                                      link.active ? 'bg-emerald-600 text-white ring-emerald-600' : link.url ? 'text-slate-900 hover:bg-slate-50' : 'cursor-not-allowed bg-slate-100 text-slate-400']"
                                  v-html="link.label"
                              />
                          </nav>
                      </div>
      ...
      <div v-if="isApproveModalOpen" class="fixed inset-0 z-[9999] flex items-center justify-center p-4">
          ...
          <div class="relative w-full max-w-md rounded-lg bg-white p-6 shadow-xl" @click.stop>
              ...
              <p class="mb-6 text-sm text-slate-600">
                  Are you sure you want to approve the request by <strong>{{ approvingRequest?.user.name }}</strong> for resolution <strong>{{ approvingRequest?.resolution.title_resolutions }}</strong>? This action will allow the user to download the document.
              </p>
              <div class="flex justify-end gap-3">
                  <button @click="isApproveModalOpen = false" class="rounded-lg bg-slate-100 px-4 py-2 font-medium text-slate-700 hover:bg-slate-200">Cancel</button>
      ...
      <div v-if="isRejectModalOpen" class="fixed inset-0 z-[9999] flex items-center justify-center p-4">
          ...
          <div class="relative w-full max-w-md rounded-lg bg-white p-6 shadow-xl" @click.stop>
              ...
              <textarea v-model="rejectionReason" rows="4" class="w-full rounded-lg border border-slate-300 p-2 shadow-sm focus:ring-2 focus:ring-red-500 focus:outline-none" placeholder="Enter rejection reason"></textarea>
      ...
      <div v-if="isDetailsModalOpen" @click.self="isDetailsModalOpen = false" class="fixed inset-0 z-[9998] flex items-center justify-center bg-slate-900/60 p-4 backdrop-blur-sm">
          <div class="relative flex h-auto max-h-[90vh] w-full max-w-4xl flex-col overflow-hidden rounded-2xl bg-white shadow-2xl">
              ...
              <div v-if="viewingRequest" class="flex flex-1 flex-col overflow-y-auto md:flex-row">
                  <div class="w-full border-r border-slate-100 bg-slate-50/50 p-8 md:w-72">
                      <div class="flex flex-col items-center text-center">
                          <div class="relative mb-4">
                              <img v-if="viewingRequest.user.profile_photo" :src="`/storage/${viewingRequest.user.profile_photo}`" class="h-32 w-32 rounded-2xl border-4 border-white object-cover shadow-md" />
                              <div v-else class="flex h-32 w-32 items-center justify-center rounded-2xl bg-gradient-to-br from-emerald-500 to-teal-600 text-3xl font-bold text-white shadow-md border-4 border-white">
                                  {{ getInitials(viewingRequest.user.name) }}
                              </div>
                              <span :class="['absolute -bottom-2 -right-2 rounded-lg px-2 py-1 text-[10px] font-bold uppercase tracking-wider shadow-sm', 
                      ...
                      <div class="mt-8 space-y-4">
                          ...
                          <div class="flex flex-col">
                              ...
                              <span class="text-sm font-medium text-slate-700">{{ computeAge(viewingRequest.user.birthdate) }} yrs old ({{ viewingRequest.user.birthdate || 'N/A' }})</span>
                          </div>
                          <div class="flex flex-col">
                  ...
                  <div class="flex-1 p-8">
                      <div :class="['mb-8 flex items-center justify-between rounded-xl px-6 py-4 border-l-4', 
                          viewingRequest.status === 'pending' ? 'bg-yellow-50 border-yellow-400 text-yellow-800' : 
                          viewingRequest.status === 'approved' ? 'bg-emerald-50 border-emerald-400 text-emerald-800' : 'bg-red-50 border-red-400 text-red-800']">
                          <div>
                      ...
                      <div class="mb-8">
                          ...
                          <div class="rounded-xl border border-slate-200 p-5 transition hover:border-emerald-200 hover:bg-emerald-50/30">
                              <p class="mb-1 text-xs font-bold text-emerald-600 uppercase">{{ viewingRequest.resolution.resolution_number }}</p>
                              <h5 class="text-lg font-bold text-slate-800 leading-snug">{{ viewingRequest.resolution.title_resolutions }}</h5>
                          </div>
                      </div>
  
                      <div>
                          <h4 class="mb-4 flex items-center gap-2 text-sm font-bold uppercase tracking-widest text-slate-400">
                              <MessageSquare class="h-4 w-4" /> Reason for Request
                          </h4>
                          <div class="relative rounded-xl bg-slate-100 p-6 italic text-slate-700">
                              ...
                              <p class="relative z-10 text-sm leading-relaxed whitespace-pre-line">{{ viewingRequest.purpose }}</p>
                          </div>
                      </div>
                  </div>
              </div>
              ...
              <div class="flex items-center justify-end gap-3 border-t border-slate-100 bg-slate-50 px-8 py-5">
                  <button @click="isDetailsModalOpen = false" class="rounded-lg px-6 py-2.5 text-sm font-bold text-slate-600 transition hover:bg-slate-200">Close</button>
                  <template v-if="viewingRequest?.status === 'pending'">
                      <button @click="openRejectModal(viewingRequest); isDetailsModalOpen = false;" class="rounded-lg bg-red-50 px-6 py-2.5 text-sm font-bold text-red-600 transition hover:bg-red-100">Deny Access</button>
                      <button @click="openApproveModal(viewingRequest); isDetailsModalOpen = false;" class="flex items-center gap-2 rounded-lg bg-emerald-600 px-6 py-2.5 text-sm font-bold text-white shadow-lg shadow-emerald-200 transition hover:bg-emerald-700 active:scale-95">
