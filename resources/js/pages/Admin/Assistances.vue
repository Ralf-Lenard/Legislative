        ...
        <main class="relative flex-1 overflow-auto">
            ...
            <div class="grid grid-cols-1 gap-6 px-8 pt-8 sm:grid-cols-2 lg:grid-cols-4">
                <div class="flex items-center justify-between rounded-lg border-l-4 border-emerald-500 bg-white p-5 shadow-lg">
                    <div><p class="text-sm font-medium text-slate-500">Total Beneficiaries</p><p class="mt-2 text-3xl font-bold text-slate-900">{{ page.props.totalAssistances }}</p></div>
                    <Users class="h-8 w-8 text-emerald-500 opacity-60"/>
                </div>
                <div class="flex items-center justify-between rounded-lg border-l-4 border-rose-500 bg-white p-5 shadow-lg">
                    <div><p class="text-sm font-medium text-slate-500">Medical Aid</p><p class="mt-2 text-3xl font-bold text-slate-900">{{ page.props.medicalCount }}</p></div>
                    <Heart class="h-8 w-8 text-rose-500 opacity-60"/>
                </div>
                <div class="flex items-center justify-between rounded-lg border-l-4 border-amber-500 bg-white p-5 shadow-lg">
                    <div><p class="text-sm font-medium text-slate-500">Legal Aid</p><p class="mt-2 text-3xl font-bold text-slate-900">{{ page.props.legalCount }}</p></div>
                ...
                <div class="flex items-center justify-between rounded-lg border-l-4 border-sky-500 bg-white p-5 shadow-lg">
                    <div><p class="text-sm font-medium text-slate-500">Scholars</p><p class="mt-2 text-3xl font-bold text-slate-900">{{ page.props.scholarCount }}</p></div>
                ...
                <div class="overflow-hidden rounded-lg border border-slate-100 bg-white shadow-lg">
                    ...
                    <table v-else class="w-full divide-y divide-slate-200">
                        <thead class="bg-slate-100/80">
                            <tr>
                                <th class="px-6 py-4 text-left text-xs font-bold text-slate-700 uppercase">Beneficiary</th>
                                <th class="px-6 py-4 text-left text-xs font-bold text-slate-700 uppercase">Barangay</th>
                                <th class="px-6 py-4 text-left text-xs font-bold text-slate-700 uppercase">School/Details</th>
                                <th class="px-6 py-4 text-left text-xs font-bold text-slate-700 uppercase">Type</th>
                                <th class="px-6 py-4 text-left text-xs font-bold text-slate-700 uppercase">Date</th>
                                <th class="px-6 py-4 text-center text-xs font-bold text-slate-700 uppercase">Actions</th>
                            </tr>
                        </thead>
                        <tbody class="divide-y divide-slate-200 bg-white">
                            <tr v-for="item in assistanceList" :key="item.id" class="transition-colors hover:bg-emerald-50/50">
                                <td class="px-6 py-4">
                                    <div class="text-sm font-bold text-slate-900">{{ item.full_name }}</div>
                                    <div class="text-xs text-slate-400 font-medium">Ref #{{ item.id }}</div>
                                </td>
                                <td class="px-6 py-4 text-sm text-slate-600 font-medium">{{ item.barangay }}</td>
                                ...
                                <td class="px-6 py-4 text-sm text-slate-600 font-medium">{{ formatDate(item.created_at) }}</td>
                                <td class="px-6 py-4 text-center">
                                    <div class="flex items-center justify-center gap-2">
                                        <button @click="openModal(item)" class="h-8 w-8 flex items-center justify-center rounded-full bg-sky-50 text-sky-600 hover:bg-sky-100 transition-colors">
                                            <Edit class="h-4 w-4"/>
                                        </button>
                                        <button @click="openDeleteDialog(item)" class="h-8 w-8 flex items-center justify-center rounded-full bg-red-50 text-red-600 hover:bg-red-100 transition-colors">
