      ...
      <div class="relative max-h-[90vh] w-full max-w-2xl scale-100 transform overflow-y-auto rounded-2xl border border-slate-100 bg-white shadow-2xl transition-all">
            ...
            <div>
              <div class="flex items-center gap-2">
                ...
                <span :class="[
                  ...
                  official.type === 'official' ? 'bg-emerald-100 text-emerald-700' : 'bg-blue-100 text-blue-700'
                ]">
                  {{ official.type }}
                </span>
              </div>
        ...
        <div class="p-8 space-y-8">
          <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
            <div v-if="official.type === 'official' && official.main_committee" class="rounded-xl border border-emerald-100 bg-emerald-50/30 p-5">
              <div class="flex items-center gap-3">
                ...
                <div>
                  <p class="text-[10px] font-bold uppercase tracking-tight text-emerald-600/70">Main Committee</p>
                  <p class="text-sm font-bold text-slate-800">{{ official.main_committee }}</p>
                </div>
              </div>
            </div>

            <div v-if="official.type === 'employee' && official.division" class="rounded-xl border border-blue-100 bg-blue-50/30 p-5">
              <div class="flex items-center gap-3">
                <div class="flex h-10 w-10 items-center justify-center rounded-lg bg-white shadow-sm">
                  <Briefcase class="h-6 w-6 text-blue-600" />
                </div>
                <div>
                  <p class="text-[10px] font-bold uppercase tracking-tight text-blue-600/70">Department / Division</p>
                  <p class="text-sm font-bold text-slate-800">{{ official.division }}</p>
                </div>
              </div>
            </div>
          </div>
          ...
          <div>
            <h3 class="mb-3 text-xs font-bold uppercase tracking-widest text-slate-400">Personnel Profile & Bio</h3>
            <div class="rounded-xl border border-slate-100 bg-slate-50/30 p-5 leading-relaxed text-slate-600 shadow-sm">
              ...
              <p v-else class="text-sm italic text-slate-400 text-center py-4">No additional profile information provided.</p>
            </div>
          </div>

          <div v-if="official.type === 'official' && official.committees?.length">
...
import { X, User, ShieldCheck, Briefcase } from 'lucide-vue-next';
