...
const props = defineProps<{
    isOpen: boolean;
    viewingRequest: ResolutionDownloadRequest | null;
}>();

const emit = defineEmits<{
    (e: 'close'): void;
    (e: 'approve', request: ResolutionDownloadRequest): void;
    (e: 'reject', request: ResolutionDownloadRequest): void;
}>();
    ...
    <div v-if="isOpen && viewingRequest" class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm">
        <div class="relative flex h-full max-h-[90vh] w-full max-w-5xl flex-col overflow-hidden rounded-2xl bg-white shadow-2xl">
            ...
            <div class="flex items-center justify-between border-b border-slate-100 px-8 py-4">
                <h2 class="text-xl font-bold text-slate-800">Resolution Request Details</h2>
                ...
                <div class="flex-1 p-8">
                    ...
                    <div class="grid grid-cols-1 gap-6 xl:grid-cols-2">
                        <div>
                            ...
                            <div class="rounded-xl border border-slate-200 bg-white p-5 transition hover:border-blue-200 hover:bg-blue-50/30">
                                <p class="mb-1 text-xs font-bold text-blue-600 uppercase">{{ viewingRequest.resolution.resolution_number }}</p>
                                <h5 class="text-base leading-snug font-bold text-slate-800">{{ viewingRequest.resolution.title_resolutions }}</h5>
                            </div>

                            <h4 class="mt-6 mb-3 flex items-center gap-2 text-sm font-bold tracking-widest text-slate-400 uppercase">
                                <MessageSquare class="h-4 w-4" /> Purpose of Request
                            </h4>
                        ...
                        <div>
                            ...
                            <div class="rounded-xl border border-slate-200 bg-slate-50 p-4">
                                <p class="mb-3 text-sm font-semibold text-slate-700">
                                    ID Type: <span class="font-bold text-blue-600">{{ viewingRequest.valid_id_type }}</span>
                                </p>
                                ...
                                <div v-else-if="viewingRequest.valid_id_url"
                                    ...
                                    <span>Document submitted. Preview not available.</span>
                                </div>
                                <div v-else
            ...
            <div class="flex items-center justify-end gap-3 border-t border-slate-100 bg-slate-50 px-8 py-5">
                ...
                <template v-if="viewingRequest.status === 'pending'">
                    ...
                    <button @click="emit('approve', viewingRequest)"
                        class="flex items-center gap-2 rounded-lg bg-blue-600 px-6 py-2.5 text-sm font-bold text-white shadow-lg shadow-blue-200 transition hover:bg-blue-700 active:scale-95">
        ...
        <div v-if="isIdModalOpen" @click="isIdModalOpen = false" class="fixed inset-0 z-[60] flex items-center justify-center bg-black/90 p-4">
            ...
            <button class="absolute top-6 right-6 text-white hover:scale-110 transition"><X class="h-8 w-8" /></button>
        </div>
    </div>
</template>