      ...
      <div class="relative max-h-[90vh] w-full max-w-2xl scale-100 transform overflow-y-auto rounded-xl border border-slate-100 bg-white shadow-2xl transition-all duration-300">
        ...
        <div class="p-8 space-y-8">
          ...
          <div>
            ...
            <div v-if="session.images && session.images.length > 0" class="grid grid-cols-2 gap-4">
              <div v-for="(img, idx) in session.images" :key="'img-'+idx" class="group relative aspect-video overflow-hidden rounded-xl border border-slate-200 bg-slate-100">
                <img 
            ...
            <div v-else class="flex flex-col items-center justify-center rounded-xl border-2 border-dashed border-slate-200 py-6 text-slate-400">
              <ImageIcon class="mb-2 h-6 w-6 opacity-20" />
              <p class="text-[10px] italic">No images attached.</p>
            </div>
          </div>

          <div>
            <h3 class="mb-4 text-xs font-bold uppercase tracking-widest text-slate-400">Session Videos</h3>
            <div v-if="session.videos && session.videos.length > 0" class="grid grid-cols-2 gap-4">
              <div v-for="(vid, vidx) in session.videos" :key="'vid-'+vidx" class="group relative aspect-video overflow-hidden rounded-xl border border-slate-200 bg-slate-900">
                <video 
                  class="h-full w-full object-cover"
                  controls
                  preload="metadata"
                >
                  <source :src="'/storage/' + vid.file_path" type="video/mp4">
                  Your browser does not support the video tag.
                </video>
                <div v-if="vid.title" class="pointer-events-none absolute top-0 left-0 right-0 bg-gradient-to-b from-slate-900/80 to-transparent p-3 opacity-0 transition-opacity group-hover:opacity-100">
                  <p class="text-[10px] font-bold text-white uppercase tracking-wider">{{ vid.title }}</p>
                </div>
              </div>
            </div>
            <div v-else class="flex flex-col items-center justify-center rounded-xl border-2 border-dashed border-slate-200 py-6 text-slate-400">
              <VideoIcon class="mb-2 h-6 w-6 opacity-20" />
              <p class="text-[10px] italic">No videos attached.</p>
            </div>
          </div>
        </div>
...
import { X, Calendar, FileText, Image as ImageIcon, Video as VideoIcon } from 'lucide-vue-next';
...
/* Optional: Subtle focus state for the video player */
video:focus {
  outline: none;
}
</style>