            ...
            <form @submit.prevent="submit" class="space-y-5">
                ...
                <div class="space-y-6">
                    <div>
                        ...
                        <div
                            v-if="form.file_path_resolutions || oldFile"
...
const selectedFileIsImage = computed(() => {
    const name = form.file_path_resolutions?.name || oldFile.value;
...
/**
 * Resets the form. Runs ONLY when the modal opens or the target resolution
 * changes — NOT on every internal state change (previewUrl, oldFile, etc).
 * A `watchEffect` here would auto-track previewUrl/oldFile too (since they're
 * read below), and re-run every time processFile() writes to them — wiping
 * out the file you just picked and any typed input.
 */
watch(
    () => [props.isOpen, props.resolution?.id],
    () => {
        ...
        if (props.resolution) {
            ...
            oldFile.value = getFileUrl(props.resolution.file_path_resolutions);
...
const submit = async () => {
    ...
    if (!form.file_path_resolutions && oldFile.value) {
