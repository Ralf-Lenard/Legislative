...
class ResolutionDownloadRequestSeeder extends Seeder
{
    /**
    ...
    public function run(): void
    {
        ...
        $resolutions = Resolution::pluck('id')->toArray(); // all resolution IDs

        $statuses = ['pending', 'approved', 'rejected'];
        $validIdTypes = ['Passport', 'Driver’s License', 'PRC ID', 'SSS ID', 'PhilHealth ID'];

        for ($i = 0; $i < 50; $i++) {
            DB::table('resolution_download_requests')->insert([
                'user_id' => $users[array_rand($users)],
                'resolution_id' => $resolutions[array_rand($resolutions)],
